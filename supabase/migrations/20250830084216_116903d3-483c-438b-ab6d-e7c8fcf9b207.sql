...
CREATE TABLE public.activities (
  ...
  activity_type TEXT DEFAULT 'running', -- running, cycling, etc
  created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
  updated_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now()
);

-- Create activity tags table
...
-- Activity tags policies
CREATE POLICY "Users can view tags for their activities" 
...
CREATE POLICY "Users can create tags for their activities" 
ON public.activity_tags 
FOR INSERT 
WITH CHECK (EXISTS (
  SELECT 1 FROM public.activities 
...
CREATE POLICY "Users can delete tags for their activities" 
ON public.activity_tags 
FOR DELETE 
USING (EXISTS (
  SELECT 1 FROM public.activities 
...
-- Create storage policies for GPX files
